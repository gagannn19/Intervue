...
export function HistoryListItem({ interview, onOpen }) {
  const hasScore = typeof interview.score === "number";
  return (
    <button onClick={() => onOpen(interview)} className="w-full text-left p-4 flex items-center gap-4 hover:bg-[var(--ink)]/3 transition-colors">
      {hasScore ? (
        ...
        <div className="w-12 h-12 rounded-full border-2 border-dashed border-[var(--ink)]/15 shrink-0" />
      )}
      ...
      <div className="text-right shrink-0 pl-3">
        {hasScore ? (
          ...
          <p className="text-xs text-[var(--ink)]/40">Pending feedback</p>
        )}
